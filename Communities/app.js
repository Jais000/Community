...
app.post('/search', async (req, res) => {
    ...
    for (var i = 0; i < comms.length; i++) {
        if (comms[i].name.toLowerCase().includes(search) == true) { searched.push(comms[i]) }
    }
    for (var i = 0; i < users.length; i++) {
        if (users[i].name.toLowerCase().includes(search) == true) { searchedusers.push(users[i]) }
    }
...
app.get('/community/:id', async (req, res, next) => {
    ...
    try {
        ...
        var status = await cql.isAdmin(req.session.iD, comm.id)
...
app.post('/create_event/:id', async (req, res) => {
    ...
    await cql.createEvent(req.params.id.split(':')[1], string)
    res.redirect('/update')
...
app.post('/forumsearch/:id', async (req, res) => {
    ...
    for (var i = 0; i < communityForums.length; i++) {
        if (communityForums[i].name.toLowerCase().includes(search) == true) { searched.push(communityForums[i]) }
    }
...
app.post('/create_user_event/:id', async (req, res) => {
    ...
    }else if(req.body.freq == 'day'){
        console.log('RUNNING DAY')
        var bod = req.body
        var sd = req.body.startDate
        var ed = req.body.endDate
        var freq = req.body.freq
        var sday = parseInt(sd.split('-')[2])
        var eday = parseInt(ed.split('-')[2])
        delete bod.startDate
        delete bod.endDate
        delete bod.freq
        
        console.log(JSON.stringify(bod))
        for(var i = sday; i < eday+1;i++){
            //console.log(eday)
            //bod.date = bod.date.split('-').splice(2,1,i.toString()).join('-')
            bod.date = bod.date.split('-')
            bod.date.pop()

            if(i>=10){bod.date.push(i.toString())}
            else{
                bod.date.push("0"+i.toString())
            }
            bod.date = bod.date.join('-')
            console.log(bod)
            await cql.createUserEvent(1, JSON.stringify(bod))
        }
    }else if(req.body.freq == 'month'){
        console.log('RUNNING MONTH')
        var bod = req.body
        var sd = req.body.startDate
        var ed = req.body.endDate
        var freq = req.body.freq
        var sm = parseInt(req.body.startDate.split('-')[1])
        var em = parseInt(req.body.endDate.split('-')[1])
        var sday = parseInt(sd.split('-')[2])
        var eday = parseInt(ed.split('-')[2])
    ...
    }    else if(req.body.freq == 'week'){
        console.log(req.body)
        var info = {Monday:0,Tuesday:0,Wednesday:0,Thursday:0,Friday:0,Saturday:0,Sunday:0}
        for(var k = 0;k<Object.values(req.body).length;k++){
            if(Object.values(req.body)[k]=='on')
        {
            info[Object.keys(req.body)[k]] = 1
        }
        }
        var bod = req.body
        var sd = req.body.startDate
...
/*         for(var j = 0;j<)
        ...
        while(d<=eday){
            ...
            {if(info[await cql.getDayOfWeek(sd)]==1)
                {
                    console.log(bod.date)
                    bod.date = bod.date.split('-')
                    bod.date.pop()
                    bod.date.push(d.toString())
                    bod.date = bod.date.join('-')
